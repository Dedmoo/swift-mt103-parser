...
`Mt103Parser` isolates block 4 when a SWIFT envelope is supplied, reads tags in order, checks `:20:`, `:32A:`, and `:59:`, then parses `:32A:` into a date, currency, and decimal amount. Multi-line tag values are flattened for the response record.
